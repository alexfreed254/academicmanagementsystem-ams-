    ...
    <div className="tr-card pending">
        ...
        <div>
          ...
          <div className="tr-adm">
            ...
            {student.mobile_number ? (
              <>
                &nbsp;&bull;&nbsp;{text(student.mobile_number)}
              </>
            ) : null}
          </div>
        </div>
      ...
      <div className="tr-body">
        ...
        <div>
          ...
          <span className="val">{text(row.created_at).slice(0, 10) || '—'}</span>
        </div>
      ...
      <div className="tr-actions">
        <button type="button" className="btn-clear" disabled title={ACTION_TITLE}>
          <i className="fas fa-check"></i> Clear Trainee
        </button>
        <button type="button" className="btn-reject" onClick={() => setRejOpen((v) => !v)}>
      ...
      <div className={`rej-form${rejOpen ? ' open' : ''}`} id={`rej-${id}`}>
        <input
          ...
          disabled
        />
        <button type="button" className="btn-rej-confirm" disabled title={ACTION_TITLE}>
      ...
      <div className="lost-panel">
        <div className="lost-panel-hdr">
          ...
          &nbsp;
          <button type="button" className="toggle-add-link" onClick={() => setAddOpen((v) => !v)}>
            + Add Item
          </button>
        </div>
        ...
        {lostItems.length > 0 ? (
          <div style={{ marginBottom: 10 }}>
            {lostItems.map((li, i) => (
              <div key={text(li.id) || i} className="lost-item-row">
          ...
          <div className="add-item-form">
            <div className="add-item-row">
              <input
                ...
                disabled
              />
              <input type="number" className="add-item-inp qty" defaultValue={1} min={1} disabled />
              <input
                type="text"
                ...
                disabled
              />
              <button type="button" className="btn-add-item" disabled title={ACTION_TITLE}>
      ...
      <span className="sr-only">{deptLabel}</span>
    </div>
  )
}
...
export default function ServiceDeptDashboardPage() {
  ...
  if (q.isLoading) {
    return (
      <PortalShell title="Service Clearance Dashboard">
        <PageSkeleton />
      </PortalShell>
    )
  }
  ...
  const title = `${config.label} Clearance Dashboard`
  const themeStyle = {
    ...
    <PortalShell title={title}>
      <div className="sd-dashboard" style={themeStyle}>
        <div className="sd-wrap">
          ...
          {pending.length > 0 ? (
            pending.map((row, i) => (
              <PendingCard key={text(row.id) || i} row={row} deptLabel={config.label} />
            ))
          ) : (
          ...
          {cleared.length > 0 ? (
            <>
              ...
              {cleared.map((row, i) => {
                ...
                const lostItems = list(row, '_lost_items')
                return (
                  <div key={text(row.id) || i} className="tr-card cleared">
                    <div className="tr-hdr">
                      <div>
                    ...
                    >
                      <span>
                        <i
                          ...
                          style={{ color: '#15803d', marginRight: 5 }}
                        ></i>
                        Cleared on <strong>{text(row.approved_at).slice(0, 10) || '—'}</strong>
                      </span>
                      {Object.keys(approver).length > 0 ? (
                        <span
                          style={{
          ...
          {rejected.length > 0 ? (
            <>
              ...
              {rejected.map((row, i) => {
                ...
                const approver = joined(row, '_approver')
                return (
                  <div key={text(row.id) || i} className="tr-card rejected">
                    <div className="tr-hdr">
                      <div>
                        <div className="tr-name">{text(student.full_name, '—')}</div>
                        <div className="tr-adm">Adm: {text(student.admission_no, '—')}</div>
                    ...
                    >
                      <span>
                        <i
                          ...
                          style={{ color: '#b91c1c', marginRight: 5 }}
                        ></i>
                        Rejected on <strong>{text(row.approved_at).slice(0, 10) || '—'}</strong>
                      </span>
                      {Object.keys(approver).length > 0 ? (
                        <span
                          style={{
